...
class CodeMirrorApp extends Torus.StyledComponent {
  init(app) {
    ...
    this.evalCode = async c => {
      try {
        // Wrap code in an async function to handle await calls
        const AsyncFunction = Object.getPrototypeOf(async function () { }).constructor;
        const asyncEval = new AsyncFunction(c);
    ...
    return jdom`
      <div>
        <div class="editor-menu">
          <button class="editor-button" title="run" onclick=${() => {
        this.commands.evalAll();
      }}>></>

          <button class="editor-button" title="reset code" onclick="${() => this.resetCode()
      }">
            x
          </button>
          <button class="editor-button" title="open in editor" onclick="${() => window.open(`https://hydra.ojack.xyz/?code=${btoa(
...
// Global loadScript function that avoids re-loading
